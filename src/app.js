...
function filterByAward(awardName) {
  return players.filter(player =>
    player.awards && player.awards.find(award => award.name === awardName)
  );
}
...
function filterByAwardxCountry(awardName, country) {
  return players.filter(player => player.country == country)
    .filter(player => player.awards
      .find(award => award.name === awardName));
}

//Progression 8 - Filter players that won atleast ______ awards, belong to ______ team and are younger than ____
function filterByNoOfAwardsxTeamxAge(noOfAwards, team, age){
  return players
    .filter(player => player.awards.length >= noOfAwards)
    .filter(player => player.team == team)
    .filter(player.age == age);
}
...
// console.log(FilterByTeamxSortByNoOfAwards('Chelsea'))

//Challenge 1 - Sort players that have won _______ award _____ times and belong to _______ country in alphabetical order of their names
function SortByNamexAwardxTimes(awardName, noOfTimes, country) {
  let playersByCountry = players.filter(player => player.country==country);
  let playersWonAwardTimes = playersByCountry.filter(player => {
    let count = 0;
  ...
  playersWonAwardTimes.sort((a, b) => a.name.localeCompare(b.name));
  return playersWonAwardTimes;
}
...
// console.log(SortByNamexOlderThan(22))
